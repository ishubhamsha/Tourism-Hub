  ...
  app.post('/save-trip', async (req, res) => {
    try {
      ...
      const trip = new Trip({
        ...
        flights: plan.flights || [], // Default to empty array if not provided
        itinerary: plan.itinerary || [], // Default to empty array if not provided
        expenses: expenses || [] // Default to empty array if not provided
      });
...
app.get('/profile', async (req, res) => {
    ...
    try {
        // Fetch saved trips for the logged-in user
        const trips = await Trip.find({ userId: req.session.userId });
        console.log('Retrieved trips:', trips); // Debugging line
...
app.post('/add-expense', (req, res) => {
  ...
  // Re-render the trip plan view, including updated expenses
